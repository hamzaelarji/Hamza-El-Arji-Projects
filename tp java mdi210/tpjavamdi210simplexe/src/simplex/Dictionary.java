...
public class Dictionary extends Observable {
	...
	public int searchFirstIndexEnteringVariable() {
		 for (int j = 1; j <= nbNonBasicVar+1; j++) {
	            if (D[0][j] > 0.0) {	                
	                return j;
	            }
	        }
	...
	public int searchIndexLeavingVariable(int jE){
		int minIndex = 0; // Initialize to an invalid index
		double minRatio = Double.MAX_VALUE; // Initialize to a large value
	...
	public void pivote(int iS, int jE) {
		double pivotElement = D[iS][jE];
		for (int j = 1; j <= nbNonBasicVar; j++) {
			if (j != jE) {
				D[iS][j] = D[iS][j]/(-pivotElement);
			}
			else {
				D[iS][jE]=1/D[iS][jE];
			}
